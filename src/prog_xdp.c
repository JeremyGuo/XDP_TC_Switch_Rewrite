...
inline int is_TCP(void *data_begin, void *data_end){
 ...
 */
inline int getHashOld(uint32_t ip_addr, uint16_t port) {
    struct ip_key key = {
        .ip = ip_addr,
        .port = port
    };
 ...
 */
inline int getHashNew(uint32_t ip_addr, uint16_t port) {
    struct ip_key key = {
        .ip = ip_addr,
        .port = port
    };
    ...
    if(!val) {
        ...
        emp.counter = 0;
        if(bpf_map_update_elem(&ip_map, &ip_addr, &emp, BPF_ANY) < 0)
            return -1;
        return 0;
    }
    int ret = ++ val->counter;
...
inline void storeIP(uint32_t ip_addr, uint16_t port, uint32_t counter, unsigned char* store_res) {
    struct flow_key key = {
        .key = {ip_addr, port},
        .idx = counter
    };
...
inline void restoreIP(uint32_t ip_addr, uint16_t port, uint32_t counter, unsigned char* store_pos) {
    struct flow_key key = {
        .key = {ip_addr, port},
        .idx = counter
    };
